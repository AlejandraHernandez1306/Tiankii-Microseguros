    ...
    <div class="py-12 bg-slate-50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-gradient-to-r from-blue-900 to-blue-700 rounded-2xl shadow-xl p-8 text-white flex justify-between items-center border-b-4 border-blue-500">
                <div>
                    <h1 class="text-3xl font-black tracking-tight">Hola, {{ Auth::user()->name }} 👋</h1>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4"> 
                    ...
                    <div class="p-4 bg-slate-50 rounded-xl border border-gray-100 hover:bg-white hover:shadow-md transition">
                        ...
                        <div class="flex items-center gap-2 mt-1">
                            ...
                            @if(str_contains(strtolower($metodo), 'bitcoin'))
                                <span class="text-orange-500 font-bold text-xl">₿</span>
                            @else
                                <span class="text-blue-500 font-bold text-xl">💳</span>
                            @endif
                            
                            <p class="text-lg font-black text-gray-700 leading-none">{{ $metodo }}</p>
                        </div>
                    </div>
            ...
            <div class="grid md:grid-cols-3 gap-8">
                ...
                <div class="md:col-span-1 space-y-6">
                        ...
                        <div class="bg-slate-800 rounded-2xl p-6 text-white shadow-2xl relative overflow-hidden">
                            ...
                            <div class="flex justify-between items-start mb-6">
                                ...
                                <div class="bg-white p-1 rounded-lg">
                                    <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=TIANKII-{{ Auth::user()->id }}" alt="QR" class="w-12 h-12">
                                </div>
                            </div>
                            <div class="mb-4">
                                <p class="text-[10px] opacity-75 uppercase font-bold">Asegurado Titular</p>
                                <p class="font-bold text-lg tracking-wide truncate uppercase">{{ Auth::user()->name }}</p>
                            </div>
                            <div class="flex justify-between text-sm pt-4 border-t border-white/10">
                                <div>
                                    ...
                                    <p class="font-bold">{{ Auth::user()->polizas->first()->nombre_plan ?? 'Estándar Rural' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-4 rounded-xl shadow border border-gray-100">
                        <a href="{{ route('contrato.ver') }}" target="_blank" class="flex items-center justify-between text-blue-900 font-black hover:text-blue-600 transition group">
                ...
                <div class="md:col-span-2">
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
                        ...
                        <div class="divide-y divide-gray-100">
                            @forelse($misAtenciones as $atencion)
                            <div class="p-6 hover:bg-gray-50 transition">
                                <div class="flex justify-between items-start mb-2">
                                    <div>
                                        ...
                                        <h4 class="font-bold text-gray-800 text-lg leading-tight">{{ $atencion->diagnostico }}</h4>
                                    </div>
                                    <span class="text-sm font-bold text-gray-400">{{ $atencion->created_at->format('d M, Y') }}</span>
                                </div>
                                
                                <p class="text-sm text-gray-600 mb-4">
                                    Tratante: <span class="font-bold text-teal-600">Dr. {{ $atencion->medico->name ?? 'Staff Médico' }}</span>
                                </p>

                                <a href="{{ route('receta.imprimir', $atencion->id) }}" target="_blank" class="inline-flex items-center gap-2 text-sm font-black text-teal-600 hover:text-teal-800 border border-teal-200 bg-teal-50 px-4 py-2 rounded-xl transition shadow-sm hover:shadow">
                                    💊 Ver Receta Médica
                                </a>
                            </div>
                            @empty
