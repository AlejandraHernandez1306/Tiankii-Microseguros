...
<body class="bg-gradient-to-br from-cyan-50 via-white to-blue-100 min-h-screen">
    ...
    <nav class="bg-white/80 backdrop-blur-md shadow-sm p-4 sticky top-0 z-50">
        <div class="max-w-7xl mx-auto flex justify-between items-center">
            ...
            <div class="space-x-4">
                @auth
                    <a href="{{ url('/dashboard') }}" class="font-bold text-teal-700 hover:text-teal-900">Ir al Panel</a>
                @else
                    <a href="{{ route('login') }}" class="font-bold text-slate-600 hover:text-teal-600">Iniciar Sesión</a>
                    <a href="{{ route('register') }}" class="bg-teal-600 text-white px-5 py-2 rounded-full font-bold hover:bg-teal-700 transition shadow-md hover:shadow-lg">Afiliarse</a>
                @endauth
            </div>
        </div>
    </nav>
    ...
    <div class="max-w-6xl mx-auto px-6 py-16 flex flex-col md:flex-row items-center gap-12">
        <div class="md:w-1/2 space-y-6">
            ...
            <div class="flex flex-wrap gap-4 pt-4">
                @auth
                    <a href="{{ url('/dashboard') }}" class="px-8 py-4 bg-teal-600 text-white rounded-xl font-bold shadow-xl hover:bg-teal-700 transition transform hover:-translate-y-1">
                        Entrar a mi Cuenta
                    </a>
                @else
                    <a href="{{ route('register') }}" class="px-8 py-4 bg-teal-600 text-white rounded-xl font-bold shadow-xl hover:bg-teal-700 transition transform hover:-translate-y-1">
                        Comenzar Ahora
                    </a>
                    <a href="{{ route('login') }}" class="px-8 py-4 bg-white text-teal-700 border-2 border-teal-100 rounded-xl font-bold shadow-sm hover:bg-teal-50 transition">
                        Soy Médico
                    </a>
                @endauth
            </div>
